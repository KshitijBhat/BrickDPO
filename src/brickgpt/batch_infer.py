...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='proj_develop/batch_outputs', help='Base directory for all outputs')
    ...
    pd.DataFrame(rejection_stats).to_csv(os.path.join(args.output_dir, 'brick_rejections_stats.csv'), index=False)
    pd.DataFrame(reasons_stats).to_csv(os.path.join(args.output_dir, 'brick_rejection_reasons.csv'), index=False)
    pd.DataFrame(regen_stats).to_csv(os.path.join(args.output_dir, 'regeneration_stats.csv'), index=False)

    print(f"Saved stats to {args.output_dir}/brick_rejections_stats.csv, brick_rejection_reasons.csv, regeneration_stats.csv")
